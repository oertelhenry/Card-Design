...
/* ============================================================================
   Majestic — restaurant menu card ("Burger Bliss"), dark theme
   Pixel-faithful rebuild of:
     CardDesigns/genericCard/geneicdarkdesktop.mp4
     CardDesigns/genericCard/genericdarkmobile.mp4

   A moody, editorial digital menu on a near-black canvas:
     • Hero      — full-bleed food image, serif wordmark, tagline
     • Nav       — Food / Drinks / Dessert tabs (orange underline active)
     • Menu      — gold script section headings + item rows (name, badges, price)
   ...
   layout also reflows automatically to the real viewport width. The sun/moon
   button flips between the dark ("Majestic") and light themes.
...
// ─── Palette (dark — "Majestic") ──────────────────────────────────────────────
const DARK = {
  page:      "#150B08",
  panel:     "#1E110C",   // alternating section panel
  panelBd:   "#301C13",
  navBg:     "#120A07",
  navBd:     "#2A1710",

  ink:       "#F1E6D6",   // item names
  head:      "#ECCF8E",   // gold script headings
  body:      "#A98C72",   // descriptions
  bodyDim:   "#8A7059",
  price:     "#E5843E",   // orange prices
  priceLbl:  "#C9BBA6",   // "Single" / "Double"
  accent:    "#DE6B34",   // active tab
  accentBd:  "#8E4426",
  accentBg:  "rgba(222,107,52,0.14)",
  veg:       "#F2E4D6",
  vegBg:     "#B58A84",
  vegBd:     "#8A625C",
  rule:      "#332016",
...
// ─── Palette (light) — for the sun/moon toggle ────────────────────────────────
const LIGHT = {
  page:      "#F5EEE4",
  panel:     "#F1E6D8",
  panelBd:   "#EADBC9",
...
const HERO_IMG =
  ...
  "linear-gradient(180deg, #241812 0%, #3A241799 40%, #100805 100%)";
...
const MENUS = {
  ...
  dessert: [
    ...
    },
    {
      title: "Cold & Fresh",
      items: [
        { name: "Tiramisu", desc: "Classic Italian coffee-flavored dessert with mascarpone and cocoa.", price: "$8.50" },
        { name: "Lemon Cheesecake", desc: "Creamy lemon cheesecake with graham cracker crust and berry compote.", price: "$7.95" },
        { name: "Affogato", desc: "Vanilla gelato drowned in a shot of hot espresso.", price: "$6.50" },
        { name: "Mixed Berry Parfait", veg: true, desc: "Layers of Greek yogurt, granola, and fresh seasonal berries.", price: "$7.50" },
      ],
    },
  ],
};
...
function TagBadge({ label, t }) {
  return (
    <span style={{
      ...
      border: `1px solid ${t.accentBd}`, color: t.accent, background: t.accentBg,
  ...
  if (item.double) {
    return (
      <div style={{ textAlign: "right", fontFamily: SANS, fontSize: 12.5, color: t.price, lineHeight: 1.55, whiteSpace: "nowrap" }}>
        <div><span style={{ color: t.priceLbl }}>Single</span> {item.single}</div>
        <div><span style={{ color: t.priceLbl }}>Double</span> {item.double}</div>
      </div>
    );
  }
  return (
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: narrow ? 12 : 16 }}>
        <span style={{ height: 1, width: narrow ? 20 : 30, background: "rgba(240,225,205,0.55)" }} />
        <span style={{ fontFamily: SANS, fontSize: narrow ? 13 : 15, fontWeight: 600, color: t.heroSub, letterSpacing: "0.01em", textShadow: "0 1px 10px rgba(0,0,0,0.6)" }}>
...
// ─── Preview controls (Desktop / Mobile pill + theme toggle) ──────────────────
function PreviewControls({ device, setDevice, dark, setDark }) {
  return (
    <div style={{ position: "absolute", top: 16, right: 16, zIndex: 20, display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{ display: "flex", background: "rgba(255,255,255,0.12)", borderRadius: 999, padding: 3, backdropFilter: "blur(6px)" }}>
        {[["desktop", "Desktop"], ["mobile", "Mobile"]].map(([id, label]) => {
          ...
          return (
            <button key={id} onClick={() => setDevice(id)} style={{
              ...
              background: on ? DARK.accent : "transparent", color: on ? "#fff" : "rgba(255,255,255,0.7)",
      ...
      <button onClick={() => setDark(v => !v)} title="Toggle theme" style={{
        ...
        background: dark ? "rgba(40,26,18,0.9)" : "rgba(255,255,255,0.9)",
        display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.3)",
      }}>
        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={dark ? "#E8B65C" : "#3A2A1E"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          {dark
...
function MenuCard({ t, narrow, device, setDevice, dark, setDark }) {
  const [tab, setTab] = useState("food");
  const sections = MENUS[tab];
  return (
    <div style={{ background: t.page, minHeight: "100%", position: "relative" }}>
      <PreviewControls device={device} setDevice={setDevice} dark={dark} setDark={setDark} />
...
//  ROOT
// ═══════════════════════════════════════════════════════════════════════════════
export default function Majestic() {
  useFontLoader();
  const autoNarrow = useViewportNarrow();
  const [dark, setDark] = useState(true); // "Majestic" defaults to the dark theme
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflowY: "auto", background: dark ? "#0A0605" : "#EFE7DB", fontFamily: SANS }}>
      {phoneFrame ? (
        <div style={{ display: "flex", justifyContent: "center", padding: "28px 16px" }}>
          <div style={{
            ...
            border: `10px solid ${t.frame}`, boxShadow: "0 30px 70px rgba(0,0,0,0.5)", background: t.page,
          }}>
            <MenuCard t={t} narrow device={device} setDevice={setDevice} dark={dark} setDark={setDark} />
          </div>
        </div>
      ) : (
        <MenuCard t={t} narrow={autoNarrow || mobile} device={device} setDevice={setDevice} dark={dark} setDark={setDark} />
      )}
    </div>
  );
}
