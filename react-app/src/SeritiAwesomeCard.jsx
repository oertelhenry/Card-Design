...
/* ══════════════════════════════════════════════════════════════
   SERITI SOLUTIONS — "Awesome" Digital Business Card
   Recreated from CardDesigns/NewCard2 screen recordings
   (AwesomeMobile.mp4, AwesomeDesktop.mp4)

   Fonts: Playfair Display (serif headings) + system sans (body)
   Palette: dark wine/bokeh hero banner, ivory/putty sections,
            burnt-orange accent, deep espresso buttons, sea-green
            company label, forest-green WhatsApp/hours accents
   ══════════════════════════════════════════════════════════════ */

const IMG = {
  banner: "/seriti-awesome/Banner.jpg",
  profile: "/seriti-awesome/Profile.jpg",
  gallery: [
    "/seriti-awesome/Gallery1.jpg",
    "/seriti-awesome/Gallery2.jpg",
    "/seriti-awesome/Gallery3.jpg",
    "/seriti-awesome/Gallery4.jpg",
    "/seriti-awesome/Gallery5.jpg",
    "/seriti-awesome/Gallery6.jpg",
  ],
};
...
const PillBtn = ({ icon, label, variant, size }) => {
  ...
  return (
    <button style={{
      ...
      borderRadius: 999, border: (solid || brown) ? "none" : `1px solid ${C.border}`,
      background: brown ? C.brown : solid ? C.green : C.bgAlt,
      color: (solid || brown) ? "#fff" : C.navy, fontSize: compact ? 12.5 : 13.5, fontWeight: 600,
      cursor: "pointer", fontFamily: C.sans, whiteSpace: "nowrap",
    }}>
      <Icon name={icon} size={14} color={(solid || brown) ? "#fff" : C.navy} /> <span>{label}</span>
    </button>
  );
};
...
const TestimonialCard = ({ t }) => (
  <div style={{ background: C.cardWhite, border: `1px solid ${C.border}`, borderRadius: 16, padding: "20px 22px 22px" }}>
    <Stars />
            ...
            {HOURS.map((row, i) => (
              <div key={row.d} style={{
                ...
                <span style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13.5, fontWeight: 600, color: row.closed ? "#9A957F" : C.navy }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: row.closed ? "#C9C2AC" : C.green }} />
                  {row.d}
                </span>
        ...
        <div style={wrap}>
          ...
          <div style={{
            ...
            <button style={{
              ...
              background: C.brown, color: "#fff", fontSize: 14.5, fontWeight: 700, cursor: "pointer",
...
/* ══════════════════════════════════════════════════════════════
   HERO WRAPPER — banner with title + overlapping profile block
   ══════════════════════════════════════════════════════════════ */
const HeroBanner = ({ desktop }) => {
  const photoSize = desktop ? 130 : 110;
  return (
    <div style={{ position: "relative", background: C.bgLight }}>
      <div style={{
        height: desktop ? 340 : 250, background: `url(${IMG.banner}) center/cover no-repeat`,
        display: "flex", alignItems: "center", justifyContent: "center",
      }}>
        <h1 style={{ fontFamily: C.serif, fontWeight: 700, fontSize: desktop ? 40 : 28, color: "#fff", margin: 0 }}>SERITI Solutions</h1>
      </div>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: desktop ? "0 20px 56px" : "0 20px 40px" }}>
        <div style={{ position: "relative", width: photoSize, height: photoSize, marginTop: -(photoSize / 2), flexShrink: 0 }}>
          <img src={IMG.profile} alt={PROFILE.name} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%", display: "block", border: "4px solid #fff" }} />
          <div style={{
            position: "absolute", bottom: -2, right: -2, width: 26, height: 26, borderRadius: "50%",
            background: C.accent, border: "2px solid #fff", display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <Icon name="check" size={12} color="#fff" sw={2.5} />
          </div>
        </div>
        <div style={{ textAlign: "center", marginTop: 14 }}>
          <h1 style={{ fontFamily: C.serif, fontWeight: 700, fontSize: desktop ? 34 : 25, color: C.navy, margin: "0 0 6px", letterSpacing: "-0.01em" }}>{PROFILE.name}</h1>
          <p style={{ fontSize: desktop ? 15.5 : 14, fontWeight: 600, color: C.navy, margin: "0 0 6px" }}>
            {PROFILE.title} <span style={{ color: C.accent }}>{PROFILE.region}</span>
          </p>
          <p style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: "0.12em", color: C.teal, margin: "0 0 16px" }}>
            {PROFILE.company}
          </p>
          <div style={{ display: "flex", gap: 9, justifyContent: "center", flexWrap: "wrap" }}>
            <PillBtn icon="mail" label="Email" size="sm" variant="brown" />
...
function MobileCard() {
  return (
    <div style={{ fontFamily: C.sans, position: "relative", background: C.bgLight }}>
      <HeroBanner desktop={false} />
      <ContentSections desktop={false} />

      {/* floating scroll-to-top */}
      <button style={{
        position: "fixed", bottom: 78, right: 18, width: 44, height: 44, borderRadius: "50%",
        background: C.brown, border: "none", display: "flex", alignItems: "center", justifyContent: "center",
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
export default function SeritiAwesomeCard() {
