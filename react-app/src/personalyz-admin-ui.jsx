      ...
      <div style={{ padding: "20px 28px", maxWidth: 1200 }}>
        ...
        {viewMode === "grid" ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
            {filtered.map((card) => (
              <div key={card.id} onClick={() => onNav(card.type === "Generic" ? "genericEditor" : "editor", card)} style={{
          ...
          <div style={{ background: T.surface, borderRadius: T.radius, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            ...
            {filtered.map((card, i) => (
              <div key={card.id} style={{ display: "grid", gridTemplateColumns: "1fr 80px 80px 100px 100px 60px", gap: 12, alignItems: "center", padding: "12px 20px", borderBottom: i < filtered.length - 1 ? `1px solid ${T.borderLight}` : "none", fontSize: 13, fontFamily: font.ui }}>
                ...
                <button onClick={() => onNav(card.type === "Generic" ? "genericEditor" : "editor", card)} style={{ width: 28, height: 28, borderRadius: 6, border: `1px solid ${T.border}`, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Icon name="edit" size={13} color={T.sub} />
                </button>
              </div>
            ))}
...
const winterMenuData = {
  id: "1b9265c9-f81d-4d6e-87ab-bfc37bbacda3",
  cardName: "WinterMenu",
  normName: "wintermenu",
  companyName: "Personalyz",
  companyId: "99711c2d-160a-4e35-9dc2-8bbe4e973855",
  orgUnitId: "0610f6e1-7220-4976-95fb-985c0e13d723",
  orgUnitName: "DemoCards",
  emailAddress: "oertel.henry@gmail.com",
  userId: "86c74b64-ec01-4c9f-ab6f-0a024789ebe4",
  theme: "classic",
  purpose: "restaurant",
  details: {
  ...
  cardBlocks: [
    {
      id: "0704ad0e-2a69-45c8-ac24-8d23758cee47",
      tabName: "Food Menu",
      tabNumber: 0,
    ...
    {
      id: "ef5e2878-474c-4efd-af2f-7fb892f70f39",
      tabName: "Drinks Menu",
      tabNumber: 1,
      title: "Smoothies",
      sort: 0,
    ...
    {
      id: "b6107080-d412-4ac8-bb19-42d0a6d508d1",
      tabName: "Dessert Menu",
      tabNumber: 2,
      title: "Warm Desserts",
      sort: 0,
      blockType: "catalog",
...
const BLOCK_TYPES = {
  catalog:  { label: "Catalog / Menu",      icon: "clipboard", color: T.warning,  colorSoft: T.warningSoft,  desc: "Item · price · description",    hasValue: true,  hasDesc: true,  valueFirst: false, valuePlaceholder: "Price / value",  valueMono: true  },
  schedule: { label: "Schedule / Program",  icon: "clock",     color: T.accent,   colorSoft: T.accentSoft,   desc: "Time · session · venue",        hasValue: true,  hasDesc: true,  valueFirst: true,  valuePlaceholder: "Time",           valueMono: false },
  property: { label: "Property / Features", icon: "home",      color: T.success,  colorSoft: T.successSoft,  desc: "Feature label · detail",        hasValue: true,  hasDesc: false, valueFirst: false, valuePlaceholder: "Detail / value", valueMono: false },
  info:     { label: "Info / Content",      icon: "edit",      color: T.purple,   colorSoft: T.purpleSoft,   desc: "Heading · body text",           hasValue: false, hasDesc: true,  valueFirst: false, valuePlaceholder: "",               valueMono: false },
  checklist:{ label: "Checklist / T&Cs",   icon: "check",     color: "#00897B",  colorSoft: "#E0F2F1",      desc: "Clause · optional notes",       hasValue: false, hasDesc: true,  valueFirst: false, valuePlaceholder: "",               valueMono: false },
  contact:  { label: "Contact / Team",      icon: "users",     color: "#E91E63",  colorSoft: "#FCE4EC",      desc: "Name · phone/email · notes",    hasValue: true,  hasDesc: true,  valueFirst: false, valuePlaceholder: "Phone / email",  valueMono: false },
  layout:   { label: "Rich Layout",         icon: "monitor",   color: "#00897B",  colorSoft: "#E0F2F1",      desc: "Freeform columns, rich text",   hasValue: false, hasDesc: false, valueFirst: false, valuePlaceholder: "",               valueMono: false },
};

const PURPOSE_PRESETS = [
  { value: "restaurant", label: "Restaurant / Café",   emoji: "🍽️", desc: "Menu with items and prices"        },
  { value: "event",      label: "Event / Conference",  emoji: "🎭", desc: "Program, sessions, speakers"       },
  { value: "property",   label: "Property / Open Home",emoji: "🏡", desc: "Features and viewing info"         },
  { value: "medical",    label: "Patient / Intake",    emoji: "🏥", desc: "Medical info and intake forms"     },
  { value: "legal",      label: "T&Cs / Legal",        emoji: "📜", desc: "Terms, conditions, agreements"     },
  { value: "vendor",     label: "Vendor / Market",     emoji: "🛒", desc: "Vendor listings and stall info"    },
  { value: "services",   label: "Services / Pricing",  emoji: "💼", desc: "Service catalogue with rates"      },
  { value: "custom",     label: "Custom",               emoji: "✨", desc: "Blank canvas — configure freely"   },
];

// ── Block-type pill (inline dropdown) ───────────────────────
function BlockTypePill({ blockType, onChange }) {
  const [open, setOpen] = useState(false);
  const bt = BLOCK_TYPES[blockType] || BLOCK_TYPES.catalog;
  return (
    <div style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          background: bt.colorSoft, border: `1px solid ${bt.color}22`,
          cursor: "pointer", fontSize: 11, fontWeight: 700,
          color: bt.color, fontFamily: font.ui, lineHeight: 1,
          transition: "all 0.12s",
        }}
      >
        <Icon name={bt.icon} size={11} color={bt.color} />
        {bt.label}
        <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={bt.color} strokeWidth="2.5"><path d="M6 9l6 6 6-6"/></svg>
      </button>
      {open && (
        <>
          ...
          <div style={{
            ...
            boxShadow: T.shadowMd, width: 240, overflow: "hidden",
          }}>
            <div style={{ padding: "8px 12px 6px", borderBottom: `1px solid ${T.borderLight}` }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: T.muted, letterSpacing: "0.08em", textTransform: "uppercase", fontFamily: font.mono }}>Block Type</span>
            </div>
            {Object.entries(BLOCK_TYPES).map(([key, def]) => (
              <button
                ...
                style={{
                  ...
                  background: blockType === key ? def.colorSoft : "transparent",
                  fontFamily: font.ui, transition: "background 0.1s",
                  borderBottom: `1px solid ${T.borderLight}`,
                }}
                onMouseEnter={e => { if (blockType !== key) e.currentTarget.style.background = T.surfaceAlt; }}
                onMouseLeave={e => { if (blockType !== key) e.currentTarget.style.background = "transparent"; }}
              >
                <div style={{ width: 28, height: 28, borderRadius: 7, background: def.colorSoft, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <Icon name={def.icon} size={14} color={def.color} />
                </div>
                <div style={{ flex: 1, textAlign: "left" }}>
                  <div style={{ fontSize: 13, fontWeight: blockType === key ? 700 : 500, color: T.ink }}>{def.label}</div>
                  <div style={{ fontSize: 11, color: T.muted }}>{def.desc}</div>
                </div>
                {blockType === key && <Icon name="check" size={14} color={def.color} />}
              </button>
            ))}
          </div>
        </>
...
// ─────────────────────────────────────────────────────────────────
// RICH LAYOUT EDITOR
// Provides freeform heading / paragraph / 2-col / 3-col / divider
// / spacer blocks with inline contentEditable formatting.
// ─────────────────────────────────────────────────────────────────
const RICH_PALETTE = [
  "#1A1A1A","#3D3D3D","#7A7A7A","#FFFFFF",
  "#0066FF","#0052CC","#7B61FF","#E91E63",
  "#D32F2F","#E8890C","#1A8917","#00897B",
];

const rlCtrlBtn = {
  width: 20, height: 20, borderRadius: 4, background: T.surface,
  border: `1px solid ${T.border}`, cursor: "pointer", padding: 0,
  display: "flex", alignItems: "center", justifyContent: "center",
};

function RichLayoutEditor({ nodes = [], onChange }) {
  ...
  const uid = () => `rl${Date.now()}${Math.random().toString(36).slice(2, 5)}`;

  // ── Mutations ───────────────────────────────────────────────────
  const upd    = (id, patch)         => onChange(nodes.map(n => n.id === id ? { ...n, ...patch } : n));
  const updCol = (id, ci, html)      => onChange(nodes.map(n => n.id !== id ? n : { ...n, children: n.children.map((c, i) => i === ci ? html : c) }));
  const del    = id                  => { onChange(nodes.filter(n => n.id !== id)); if (focusedId === id) setFocusedId(null); };
  const mov    = (id, d)             => {
    const i = nodes.findIndex(n => n.id === id);
    if (i + d < 0 || i + d >= nodes.length) return;
    const a = [...nodes]; [a[i], a[i + d]] = [a[i + d], a[i]]; onChange(a);
  };

  const ins = (type) => {
    const g = { align: "left", color: T.ink2 };
    const map = {
      heading:  { id: uid(), type: "heading",  html: "New Heading",    align: "left", color: T.ink,  level: 2 },
      text:     { id: uid(), type: "text",     html: "Start typing\u2026", ...g },
      "2col":   { id: uid(), type: "columns",  cols: 2, children: ["Left column",  "Right column"] },
      "3col":   { id: uid(), type: "columns",  cols: 3, children: ["Column 1", "Column 2", "Column 3"] },
      divider:  { id: uid(), type: "divider",  color: T.border },
      spacer:   { id: uid(), type: "spacer",   height: 16 },
    };
    const node = map[type]; if (!node) return;
    const arr = [...nodes];
    arr.splice(addAt === null ? arr.length : addAt + 1, 0, node);
    onChange(arr);
    setAddAt(null);
  };

  // ── execCommand helpers — onMouseDown + preventDefault keeps
  //    contentEditable focused while clicking toolbar buttons ──────
  const fmt = (cmd, val = null) => document.execCommand(cmd, false, val);

  const activeNode = nodes.find(n => n.id === focusedId);

  // ── Add-block popup ─────────────────────────────────────────────
  const ADD_TYPES = [
    { k: "heading",  label: "Heading",    sym: "H",  c: T.accent  },
    { k: "text",     label: "Paragraph",  sym: "¶",  c: T.ink     },
    { k: "2col",     label: "2 Columns",  sym: "▥",  c: T.success },
    { k: "3col",     label: "3 Columns",  sym: "▦",  c: T.purple  },
    { k: "divider",  label: "Divider",    sym: "—",  c: T.muted   },
    { k: "spacer",   label: "Spacer",     sym: "↕",  c: T.muted   },
  ];

  const AddBtn = ({ idx }) => {
    const open = addAt === idx;
    return (
      <div style={{ position: "relative", display: "flex", justifyContent: "center", padding: "3px 0" }}>
        <button
          onMouseDown={e => { e.preventDefault(); setAddAt(open ? null : idx); }}
          style={{
            width: 22, height: 22, borderRadius: 11,
            background: open ? T.accent : T.faint, border: "none", cursor: "pointer",
            fontSize: 16, color: open ? "#fff" : T.muted, lineHeight: 1,
            display: "flex", alignItems: "center", justifyContent: "center",
            transition: "all 0.12s",
          }}
          title="Add block"
        ...
        {open && (
          <>
            <div style={{ position: "fixed", inset: 0, zIndex: 49 }} onMouseDown={() => setAddAt(null)} />
            <div style={{
              position: "absolute", top: "calc(100% + 4px)", left: "50%",
              transform: "translateX(-50%)", zIndex: 50,
              background: T.surface, border: `1px solid ${T.border}`,
              borderRadius: T.radius, boxShadow: T.shadowMd,
              padding: 8, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, width: 220,
            }}>
              <div style={{ gridColumn: "1/-1", fontSize: 10, fontWeight: 700, color: T.muted, letterSpacing: "0.08em", textTransform: "uppercase", fontFamily: font.mono, paddingBottom: 6, borderBottom: `1px solid ${T.borderLight}`, marginBottom: 2 }}>Insert block</div>
              {ADD_TYPES.map(t => (
                <button
                  key={t.k}
                  onMouseDown={e => { e.preventDefault(); ins(t.k); }}
                  style={{
                    display: "flex", alignItems: "center", gap: 7, padding: "8px 10px",
                    border: `1px solid ${T.border}`, borderRadius: T.radiusSm,
                    background: T.surface, cursor: "pointer",
                    fontFamily: font.ui, fontSize: 12, color: T.ink, transition: "all 0.1s",
                  }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = t.c; e.currentTarget.style.background = T.surfaceAlt; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = T.border; e.currentTarget.style.background = T.surface; }}
                >
                  <span style={{ fontSize: 14, color: t.c, fontWeight: 700, width: 14, textAlign: "center", flexShrink: 0 }}>{t.sym}</span>
                  {t.label}
                </button>
              ))}
            </div>
  ...
  const FormatBar = () => (
    <div style={{
      ...
      background: T.surface, position: "sticky", top: 0, zIndex: 8,
      minHeight: 42,
    }}>
      {!focusedId ? (
        <span style={{ fontSize: 11, color: T.muted, fontFamily: font.ui, fontStyle: "italic" }}>
          Click any text block to activate the format toolbar
        </span>
      ) : (
        <>
          {/* Bold / Italic / Underline */}
          {[
            { lbl: "B", cmd: "bold",      style: { fontWeight: 800 } },
            { lbl: "I", cmd: "italic",    style: { fontStyle: "italic" } },
            { lbl: "U", cmd: "underline", style: { textDecoration: "underline" } },
          ].map(({ lbl, cmd, style: s }) => (
            <button key={cmd} onMouseDown={e => { e.preventDefault(); fmt(cmd); }} title={cmd.charAt(0).toUpperCase() + cmd.slice(1)}
              style={{ ...rlCtrlBtn, width: 28, height: 28, fontFamily: font.ui, fontSize: 13, color: T.ink, ...s }}>
              {lbl}
            </button>
          ))}
          ...
          {/* Align */}
          {[
            { title: "Align left",   cmd: "justifyLeft",   sym: "≡" },
            { title: "Align center", cmd: "justifyCenter", sym: "≡" },
            { title: "Align right",  cmd: "justifyRight",  sym: "≡" },
          ].map((a, ai) => (
            <button key={a.cmd} onMouseDown={e => { e.preventDefault(); fmt(a.cmd); }} title={a.title}
              style={{ ...rlCtrlBtn, width: 28, height: 28, fontSize: 13, fontFamily: font.mono, color: T.ink }}>
              {ai === 0 ? "◀" : ai === 1 ? "⬛" : "▶"}
            </button>
          ))}
          <div style={{ width: 1, height: 20, background: T.border, margin: "0 2px" }} />
          ...
          <select
            ...
            defaultValue="3"
            style={{ padding: "3px 6px", fontSize: 11, border: `1px solid ${T.border}`, borderRadius: T.radiusSm, fontFamily: font.ui, color: T.ink, background: T.surface, cursor: "pointer", height: 28 }}
            title="Font size"
          >
            {[[1,10],[2,12],[3,14],[4,16],[5,18],[6,22],[7,28]].map(([v,px]) => (
              <option key={v} value={v}>{px}px</option>
            ))}
          </select>
          <div style={{ width: 1, height: 20, background: T.border, margin: "0 2px" }} />
          ...
          <div style={{ position: "relative" }}>
            <button
              ...
              title="Text colour"
              style={{ ...rlCtrlBtn, width: "auto", padding: "0 8px", gap: 5, height: 28, fontSize: 12, fontFamily: font.ui }}
            >
              <span style={{ fontWeight: 800, fontSize: 13, color: T.ink }}>A</span>
              <span style={{ display: "block", width: 14, height: 3, background: "#0066FF", borderRadius: 2 }} />
              <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke={T.sub} strokeWidth="2.5"><path d="M6 9l6 6 6-6"/></svg>
            </button>
            {showColors && (
              <>
                <div style={{ position: "fixed", inset: 0, zIndex: 49 }} onMouseDown={e => { e.preventDefault(); setShowColors(false); }} />
                <div style={{
                  position: "absolute", top: "calc(100% + 4px)", left: 0, zIndex: 50,
                  background: T.surface, border: `1px solid ${T.border}`,
                  borderRadius: T.radius, padding: 10, boxShadow: T.shadowMd, width: 164,
                }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, letterSpacing: "0.07em", textTransform: "uppercase", fontFamily: font.mono, marginBottom: 8 }}>Colour palette</div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 5, marginBottom: 10 }}>
                    {RICH_PALETTE.map(c => (
                      <button
                        key={c}
                        onMouseDown={e => { e.preventDefault(); fmt("foreColor", c); setShowColors(false); }}
                        style={{ width: "100%", aspectRatio: "1", borderRadius: 5, background: c, border: c === "#FFFFFF" ? `1px solid ${T.border}` : `1px solid ${c}66`, cursor: "pointer" }}
                        title={c}
                      />
                    ))}
                  </div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, letterSpacing: "0.07em", textTransform: "uppercase", fontFamily: font.mono, marginBottom: 4 }}>Custom</div>
                  <input
                    type="color"
                    defaultValue="#0066FF"
                    onInput={e => fmt("foreColor", e.target.value)}
                    style={{ width: "100%", height: 28, border: "none", cursor: "pointer", borderRadius: T.radiusSm, padding: 0 }}
                  />
                </div>
              </>
            )}
          ...
          {/* Heading level — only for heading nodes */}
          {activeNode?.type === "heading" && (
            <select
              value={activeNode.level || 2}
              onChange={e => upd(focusedId, { level: Number(e.target.value) })}
              style={{ padding: "3px 6px", fontSize: 11, border: `1px solid ${T.border}`, borderRadius: T.radiusSm, fontFamily: font.ui, color: T.ink, background: T.surface, cursor: "pointer", height: 28 }}
              title="Heading level"
            >
          ...
          <div style={{ flex: 1 }} />
          <span style={{ fontSize: 11, color: T.muted, fontFamily: font.ui }}>
            {activeNode ? `${activeNode.type === "heading" ? "Heading" : activeNode.type === "text" ? "Paragraph" : `Columns · col ${(focusedCol ?? 0) + 1}`} selected` : ""}
          </span>
        </>
      )}
    </div>
  ...
  return (
    <div>
      ...
      <div style={{ padding: "12px 16px 20px" }}>
        ...
        {nodes.length === 0 && (
          <div style={{ textAlign: "center", padding: "32px 20px", color: T.muted, fontSize: 12, fontFamily: font.ui }}>
            <div style={{ fontSize: 32, marginBottom: 10 }}>✏️</div>
            <div style={{ fontWeight: 700, color: T.ink2, marginBottom: 4, fontSize: 14 }}>Rich layout canvas</div>
            <div>Click the <strong style={{ color: T.accent }}>+</strong> button to add headings, paragraphs, columns, and more</div>
          </div>
        )}

        {nodes.map((node, idx) => {
          const isF = focusedId === node.id;
          const isH = hoveredId === node.id;
          return (
            <div key={node.id}>
              {/* ── block wrapper with hover/focus ring ── */}
              <div
                onMouseEnter={() => setHoveredId(node.id)}
                onMouseLeave={() => setHoveredId(null)}
                style={{
                  position: "relative", borderRadius: T.radiusSm,
                  border: `1.5px solid ${isF ? T.accent : isH ? T.border : "transparent"}`,
                  background: isF ? "rgba(0,102,255,0.025)" : "transparent",
                  padding: "4px 40px 4px 10px",
                  transition: "border-color 0.12s, background 0.12s",
                  minHeight: 30,
                }}
              >
                {/* ── per-block controls (visible on hover / focus) ── */}
                <div style={{
                  position: "absolute", right: 4, top: "50%", transform: "translateY(-50%)",
                  display: "flex", flexDirection: "column", gap: 2,
                  opacity: isH || isF ? 1 : 0, transition: "opacity 0.12s",
                }}>
                  <button onMouseDown={e => { e.preventDefault(); mov(node.id, -1); }} disabled={idx === 0}
                    style={{ ...rlCtrlBtn, opacity: idx === 0 ? 0.3 : 1 }} title="Move up">
                    <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={T.sub} strokeWidth="2.5"><path d="M18 15l-6-6-6 6"/></svg>
                  </button>
                  <button onMouseDown={e => { e.preventDefault(); mov(node.id, 1); }} disabled={idx === nodes.length - 1}
                    style={{ ...rlCtrlBtn, opacity: idx === nodes.length - 1 ? 0.3 : 1 }} title="Move down">
                    <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={T.sub} strokeWidth="2.5"><path d="M6 9l6 6 6-6"/></svg>
                  </button>
                  <button onMouseDown={e => { e.preventDefault(); del(node.id); }} style={rlCtrlBtn} title="Delete">
                    <Icon name="trash" size={10} color={T.danger} />
                  </button>
                </div>
                ...
                {node.type === "heading" && (
                  <div
                    ...
                    onFocus={() => { setFocusedId(node.id); setFocusedCol(null); }}
                    onBlur={e => { upd(node.id, { html: e.currentTarget.innerHTML }); }}
                    style={{
                      outline: "none", minHeight: 28, padding: "2px 0",
                      fontSize: hSize[node.level || 2], fontFamily: font.display,
                      fontWeight: 700, color: node.color || T.ink, lineHeight: 1.3,
                    }}
                  />
                )}
                ...
                {node.type === "text" && (
                  <div
                    ...
                    onBlur={e => { upd(node.id, { html: e.currentTarget.innerHTML }); }}
                    style={{
                      outline: "none", minHeight: 24, padding: "2px 0",
                      fontSize: 13, fontFamily: font.ui,
                      color: node.color || T.ink2, lineHeight: 1.6,
                    }}
                  />
                )}
                ...
                {node.type === "columns" && (
                  <div style={{
                    ...
                    {node.children.map((colHtml, ci) => (
                      <div
                        key={ci}
                        style={{
                          border: `1px solid ${isF && focusedCol === ci ? T.accent : T.borderLight}`,
                          borderRadius: T.radiusSm, padding: "8px 10px", minHeight: 56,
                          background: isF && focusedCol === ci ? "rgba(0,102,255,0.025)" : T.surfaceAlt,
                      ...
                      >
                        <div style={{
                          ...
                          fontFamily: font.mono, marginBottom: 5, userSelect: "none",
                        }}>
                          Col {ci + 1}
                        </div>
                        <div
                          ...
                          onFocus={() => { setFocusedId(node.id); setFocusedCol(ci); }}
                          onBlur={e => { updCol(node.id, ci, e.currentTarget.innerHTML); }}
                          style={{
                            outline: "none", minHeight: 28, fontSize: 13,
                            fontFamily: font.ui, color: T.ink2, lineHeight: 1.55,
                          }}
                        />
                      </div>
                    ))}
                ...
                {node.type === "divider" && (
                  <div style={{ padding: "8px 0", display: "flex", alignItems: "center", gap: 8 }}>
                    <hr style={{ flex: 1, border: "none", borderTop: `1px solid ${node.color || T.border}`, margin: 0 }} />
                    {isH && (
                      <button
                        onMouseDown={e => {
                          e.preventDefault();
                          const c = prompt("Divider colour (hex):", node.color || T.border);
                          if (c && c.startsWith("#")) upd(node.id, { color: c });
                        }}
                        style={{ fontSize: 10, color: T.muted, fontFamily: font.ui, background: "none", border: `1px solid ${T.border}`, cursor: "pointer", padding: "2px 6px", borderRadius: 3, whiteSpace: "nowrap" }}
                      >
                        edit colour
                      </button>
                    )}
                ...
                {node.type === "spacer" && (
                  <div style={{
                    ...
                    padding: "4px 0", height: node.height + 8,
                  }}>
                    <div style={{ flex: 1, borderTop: `1px dashed ${T.border}` }} />
                    <span style={{ fontSize: 10, color: T.muted, fontFamily: font.mono, whiteSpace: "nowrap", userSelect: "none" }}>
                      Spacer · {node.height}px
                    </span>
                    {isH && (
                      <select
                        onMouseDown={e => e.stopPropagation()}
                        value={node.height}
                        onChange={e => upd(node.id, { height: Number(e.target.value) })}
                        style={{ fontSize: 10, border: `1px solid ${T.border}`, borderRadius: 3, padding: "1px 4px", fontFamily: font.ui, color: T.ink, background: T.surface, cursor: "pointer" }}
                      >
                        {[8,12,16,24,32,48,64].map(h => <option key={h} value={h}>{h}px</option>)}
                      </select>
                    )}
                    <div style={{ flex: 1, borderTop: `1px dashed ${T.border}` }} />
                  </div>
...
function GenericCardEditor({ card, onNav }) {
  const [cardData, setCardData] = useState(winterMenuData);
  const [leftTab, setLeftTab] = useState("cardDetails"); // "cardDetails" | "contentBlocks"
  const [contentBlocksEnabled, setContentBlocksEnabled] = useState(true);
  ...
  // Derive unique tabs from cardBlocks
  const tabs = (() => {
    const map = {};
    cardData.cardBlocks.forEach(b => {
      if (!map[b.tabNumber]) map[b.tabNumber] = { tabNumber: b.tabNumber, tabName: b.tabName };
    });
    return Object.values(map).sort((a, b) => a.tabNumber - b.tabNumber);
  })();

  const activeTabId = selectedTabId ?? tabs[0]?.tabNumber;
  const tabSections = cardData.cardBlocks
    .filter(b => b.tabNumber === activeTabId)
    .sort((a, b) => a.sort - b.sort);

  const toggleSection = (id) => setExpandedSections(prev => ({ ...prev, [id]: !prev[id] }));

  const updateDetails = (field, value) => {
    setCardData(prev => ({ ...prev, details: { ...prev.details, [field]: value } }));
  };
  ...
  const addTab = () => {
    const newTabNumber = Math.max(...tabs.map(t => t.tabNumber)) + 1;
    const newBlock = {
      id: `new-${Date.now()}`,
    ...
    setCardData(prev => ({ ...prev, cardBlocks: [...prev.cardBlocks, newBlock] }));
    setSelectedTabId(newTabNumber);
  };

  const removeTab = (tabNumber) => {
    setCardData(prev => ({ ...prev, cardBlocks: prev.cardBlocks.filter(b => b.tabNumber !== tabNumber) }));
    if (activeTabId === tabNumber) setSelectedTabId(tabs.find(t => t.tabNumber !== tabNumber)?.tabNumber ?? null);
  };

  const addSection = () => {
    const existingSorts = tabSections.map(s => s.sort);
    const maxSort = existingSorts.length ? Math.max(...existingSorts) : -1;
    const newBlock = {
      id: `sec-${Date.now()}`,
      tabName: tabs.find(t => t.tabNumber === activeTabId)?.tabName || "Tab",
      tabNumber: activeTabId,
      title: "New Section",
      sort: maxSort + 1,
      propertyRows: [],
      layoutNodes: [],
  ...
  const updateSectionLayoutNodes = (sectionId, layoutNodes) => {
    setCardData(prev => ({
      ...prev,
      cardBlocks: prev.cardBlocks.map(b => b.id === sectionId ? { ...b, layoutNodes } : b),
    }));
  };

  const deleteSection = (sectionId) => {
    setCardData(prev => ({ ...prev, cardBlocks: prev.cardBlocks.filter(b => b.id !== sectionId) }));
  };

  const updateSectionTitle = (sectionId, title) => {
    setCardData(prev => ({
      ...prev,
      cardBlocks: prev.cardBlocks.map(b => b.id === sectionId ? { ...b, title } : b),
    }));
  };

  const addItem = (sectionId) => {
    const section = cardData.cardBlocks.find(b => b.id === sectionId);
    const bt = BLOCK_TYPES[section?.blockType || "catalog"];
    setCardData(prev => ({
      ...prev,
      cardBlocks: prev.cardBlocks.map(b => b.id === sectionId ? {
        ...b,
        propertyRows: [...b.propertyRows, {
          id: `item-${Date.now()}`,
          heading: "",
          ...(bt.hasValue ? { headingValue: "" } : {}),
          ...(bt.hasDesc  ? { headingDescription: "" } : {}),
          sort: b.propertyRows.length,
        }],
      } : b),
    }));
  };

  const updateSectionBlockType = (sectionId, blockType) => {
    setCardData(prev => ({
      ...prev,
      cardBlocks: prev.cardBlocks.map(b => b.id === sectionId ? { ...b, blockType } : b),
    }));
  };

  const updateCardPurpose = (purpose) => {
    setCardData(prev => ({ ...prev, purpose }));
  ...
  const updateItem = (sectionId, itemId, field, value) => {
    setCardData(prev => ({
      ...
      cardBlocks: prev.cardBlocks.map(b => b.id === sectionId ? {
        ...b,
        propertyRows: b.propertyRows.map(r => r.id === itemId ? { ...r, [field]: value } : r),
      } : b),
    }));
  };
  ...
  const deleteItem = (sectionId, itemId) => {
    setCardData(prev => ({
      ...
      cardBlocks: prev.cardBlocks.map(b => b.id === sectionId ? {
        ...b,
        propertyRows: b.propertyRows.filter(r => r.id !== itemId),
      } : b),
    }));
  };
  ...
  const moveItem = (sectionId, itemId, dir) => {
    setCardData(prev => ({
      ...prev,
      cardBlocks: prev.cardBlocks.map(b => {
        if (b.id !== sectionId) return b;
        const rows = [...b.propertyRows].sort((a, c) => a.sort - c.sort);
        const idx = rows.findIndex(r => r.id === itemId);
        const swapIdx = idx + dir;
        if (swapIdx < 0 || swapIdx >= rows.length) return b;
        const newRows = [...rows];
        [newRows[idx], newRows[swapIdx]] = [newRows[swapIdx], newRows[idx]];
        return { ...b, propertyRows: newRows.map((r, i) => ({ ...r, sort: i })) };
      }),
    }));
  };

  // Preview: build a small phone mockup showing the card
  const purposeInfo = PURPOSE_PRESETS.find(p => p.value === cardData.purpose) || PURPOSE_PRESETS[PURPOSE_PRESETS.length - 1];
  const PreviewPhone = () => (
    <div style={{ width: 280, flexShrink: 0, background: T.surfaceAlt, borderLeft: `1px solid ${T.border}`, display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 16px", overflowY: "auto" }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: T.ink, fontFamily: font.ui, marginBottom: 4, alignSelf: "flex-start" }}>Preview</div>
      <div style={{ fontSize: 11, color: T.muted, fontFamily: font.ui, marginBottom: 14, alignSelf: "flex-start" }}>
        {purposeInfo.emoji} {purposeInfo.label}
      </div>
      <div style={{
        width: 220, background: "#fff", borderRadius: 24, boxShadow: "0 8px 40px rgba(0,0,0,0.18), 0 0 0 8px #1a1a1a",
        overflow: "hidden", fontSize: 11, fontFamily: font.ui,
      }}>
        {/* Phone notch */}
        ...
        {/* Logo/Hero */}
        <div style={{ height: 90, background: "linear-gradient(135deg, #b8860b, #8B4513)", position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {cardData.details.logo ? (
            <img src={cardData.details.logo} alt="logo" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 0.9 }} onError={e => { e.target.style.display = "none"; }} />
          ) : (
            <span style={{ fontSize: 26, color: "#fff" }}>{purposeInfo.emoji}</span>
          )}
        </div>
        {/* Card details */}
        <div style={{ padding: "10px 12px", textAlign: "center", borderBottom: `1px solid ${T.borderLight}` }}>
          <div style={{ fontSize: 13, fontWeight: 700, fontFamily: "'Georgia', serif", color: "#2c1810", letterSpacing: "0.02em" }}>{cardData.cardName}</div>
          {cardData.details.description && <div style={{ fontSize: 10, color: T.sub, marginTop: 2, fontStyle: "italic" }}>{cardData.details.description}</div>}
          {cardData.details.enquireButton && (
            <div style={{ marginTop: 7, display: "inline-block", padding: "3px 10px", border: "1px solid #8B4513", borderRadius: 2, fontSize: 9, fontWeight: 700, letterSpacing: "0.06em", color: "#8B4513", textTransform: "uppercase" }}>
              {cardData.details.enquireButtonText || "Contact Us"}
            </div>
          )}
        </div>
        {/* Tabs */}
        <div style={{ display: "flex", overflowX: "auto", borderBottom: `1px solid ${T.borderLight}`, background: "#fafafa" }}>
          {tabs.map(tab => (
            <div key={tab.tabNumber} style={{
              padding: "5px 9px", fontSize: 9, fontWeight: 600, whiteSpace: "nowrap", cursor: "pointer",
              color: tab.tabNumber === activeTabId ? "#8B4513" : T.muted,
              borderBottom: tab.tabNumber === activeTabId ? "2px solid #8B4513" : "2px solid transparent",
            }} onClick={() => setSelectedTabId(tab.tabNumber)}>
              {tab.tabName}
            </div>
          ))}
        </div>
        ...
        <div style={{ maxHeight: 280, overflowY: "auto", padding: "8px 10px" }}>
          {tabSections.map(sec => {
            const bt = BLOCK_TYPES[sec.blockType || "catalog"];
            return (
              <div key={sec.id} style={{ marginBottom: 10 }}>
                {/* Section title with block type indicator */}
                <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 5, paddingBottom: 4, borderBottom: "1px solid #e8d5b0" }}>
                  <div style={{ width: 12, height: 12, borderRadius: 3, background: bt.colorSoft, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Icon name={bt.icon} size={8} color={bt.color} />
                  </div>
                  {sec.title && <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.05em", textTransform: "uppercase", color: "#2c1810", fontFamily: "'Georgia', serif", flex: 1 }}>{sec.title}</div>}
                </div>
                {sec.propertyRows.slice(0, 4).map(row => (
                  <div key={row.id} style={{ marginBottom: 5 }}>
                    {/* Render differently based on block type */}
                    {(sec.blockType === "catalog" || !sec.blockType) && (
                      <>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                          <span style={{ fontSize: 10, fontWeight: 600, color: "#2c1810" }}>{row.heading}</span>
                          <span style={{ fontSize: 9, fontWeight: 700, color: "#8B4513", fontFamily: font.mono }}>{row.headingValue}</span>
                        </div>
                        {row.headingDescription && <div style={{ fontSize: 9, color: T.muted, lineHeight: 1.3, marginTop: 1 }}>{row.headingDescription.substring(0, 55)}{row.headingDescription.length > 55 ? "…" : ""}</div>}
                      </>
                    )}
                    {sec.blockType === "schedule" && (
                      <div style={{ display: "flex", gap: 5 }}>
                        <span style={{ fontSize: 9, fontWeight: 700, color: T.accent, whiteSpace: "nowrap", minWidth: 32 }}>{row.headingValue || "—"}</span>
                        <div>
                          <div style={{ fontSize: 10, fontWeight: 600, color: "#2c1810" }}>{row.heading}</div>
                          {row.headingDescription && <div style={{ fontSize: 9, color: T.muted }}>{row.headingDescription.substring(0, 40)}{row.headingDescription.length > 40 ? "…" : ""}</div>}
                        </div>
                      </div>
                    )}
                    {sec.blockType === "property" && (
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ fontSize: 10, color: T.sub }}>{row.heading}</span>
                        <span style={{ fontSize: 10, fontWeight: 600, color: "#2c1810" }}>{row.headingValue}</span>
                      </div>
                    )}
                    {(sec.blockType === "info" || sec.blockType === "checklist") && (
                      <div style={{ display: "flex", gap: 5, alignItems: "flex-start" }}>
                        {sec.blockType === "checklist" && <span style={{ fontSize: 10, color: "#00897B", marginTop: 1 }}>✓</span>}
                        <div>
                          <div style={{ fontSize: 10, fontWeight: 600, color: "#2c1810" }}>{row.heading}</div>
                          {row.headingDescription && <div style={{ fontSize: 9, color: T.muted, lineHeight: 1.3, marginTop: 1 }}>{row.headingDescription.substring(0, 55)}{row.headingDescription.length > 55 ? "…" : ""}</div>}
                        </div>
                    ...
                    {sec.blockType === "contact" && (
                      <div>
                        <div style={{ fontSize: 10, fontWeight: 600, color: "#2c1810" }}>{row.heading}</div>
                        {row.headingValue && <div style={{ fontSize: 9, color: "#E91E63", fontFamily: font.mono }}>{row.headingValue}</div>}
                        {row.headingDescription && <div style={{ fontSize: 9, color: T.muted }}>{row.headingDescription.substring(0, 40)}…</div>}
                      </div>
                    )}
                  </div>
                ))}
                {sec.propertyRows.length > 4 && <div style={{ fontSize: 9, color: T.muted, textAlign: "center" }}>+{sec.propertyRows.length - 4} more</div>}
              </div>
            );
          })}
          {tabSections.length === 0 && <div style={{ textAlign: "center", color: T.muted, fontSize: 10, padding: "16px 0" }}>No sections yet</div>}
        </div>
      </div>

      {/* Block type legend */}
      <div style={{ marginTop: 18, alignSelf: "stretch" }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, letterSpacing: "0.07em", textTransform: "uppercase", fontFamily: font.mono, marginBottom: 8 }}>Block Types in use</div>
        {[...new Set(tabSections.map(s => s.blockType || "catalog"))].map(btKey => {
          const bt = BLOCK_TYPES[btKey];
          if (!bt) return null;
          return (
            <div key={btKey} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
              <div style={{ width: 20, height: 20, borderRadius: 5, background: bt.colorSoft, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Icon name={bt.icon} size={11} color={bt.color} />
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: T.ink, fontFamily: font.ui }}>{bt.label}</div>
                <div style={{ fontSize: 10, color: T.muted, fontFamily: font.ui }}>{bt.desc}</div>
              </div>
            </div>
          );
        })}
        {tabSections.length === 0 && <div style={{ fontSize: 11, color: T.muted, fontFamily: font.ui, fontStyle: "italic" }}>Add sections to see block types</div>}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <TopBar
        title="Edit Generic Card"
        subtitle={`${cardData.cardName} · ${cardData.companyName} · Generic Card`}
        breadcrumb={[
          { label: "All Cards", onClick: () => onNav("cards") },
      ...
      <div style={{ display: "flex", height: "calc(100vh - 56px)", overflow: "hidden" }}>
        ...
        <div style={{ width: 200, borderRight: `1px solid ${T.border}`, background: T.surface, display: "flex", flexDirection: "column", flexShrink: 0 }}>
          {/* Tab strip */}
          <div style={{ padding: "12px 12px 0", borderBottom: `1px solid ${T.borderLight}` }}>
            <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T.muted, marginBottom: 8, fontFamily: font.mono }}>Card Sections</div>
            {["cardDetails", "contentBlocks"].map(t => (
              <button key={t} onClick={() => setLeftTab(t)} style={{
                display: "block", width: "100%", textAlign: "left", padding: "7px 10px",
                borderRadius: T.radiusSm, border: "none", cursor: "pointer", marginBottom: 2,
                background: leftTab === t ? T.accentSoft : "transparent",
                color: leftTab === t ? T.accent : T.ink, fontFamily: font.ui,
                fontSize: 13, fontWeight: leftTab === t ? 700 : 500, transition: "all 0.12s",
              }}>
                {t === "cardDetails" ? "Card Details" : "Content Blocks"}
              </button>
            ))}
            {leftTab === "contentBlocks" && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingRight: 4, paddingBottom: 2 }}>
                <span style={{ fontSize: 11, color: T.muted, fontFamily: font.ui }}>Visible</span>
                <Toggle on={contentBlocksEnabled} onChange={() => setContentBlocksEnabled(p => !p)} />
              </div>
            )}
          </div>
          ...
          {leftTab === "contentBlocks" && (
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 0" }}>
              {/* Section nav: Theme / Card Details / Content Blocks / Style */}
              {[
                { key: "theme", label: "Theme", icon: "star" },
                { key: "cardDetails", label: "Card Details", icon: "clipboard" },
                { key: "contentBlocks", label: "Content Blocks", icon: "cards" },
                { key: "style", label: "Style", icon: "monitor" },
              ].map(item => (
                <button key={item.key} onClick={() => {}} style={{
                  display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 14px",
                  border: "none", background: "transparent", cursor: "pointer", fontFamily: font.ui,
                  fontSize: 13, color: T.ink2, fontWeight: 500,
                }}>
                  <Icon name={item.icon} size={14} color={T.muted} />
                  {item.label}
                </button>
              ))}
            </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
          ...
          {leftTab === "cardDetails" && (
            <div style={{ padding: "24px 28px", maxWidth: 760 }}>
              ...
              {/* ── CARD PURPOSE ── */}
              <div style={{ background: T.surface, borderRadius: T.radius, border: `1px solid ${T.border}`, padding: 24, marginBottom: 20 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: T.accentSoft, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Icon name="cards" size={14} color={T.accent} />
                    </div>
                    <div>
                      <h2 style={{ fontSize: 15, fontWeight: 700, color: T.ink, fontFamily: font.display, margin: 0 }}>Card Purpose</h2>
                      <div style={{ fontSize: 11, color: T.muted, fontFamily: font.ui, marginTop: 1 }}>Choose what this generic card is for — shapes section defaults & preview context</div>
                    </div>
                  </div>
                  {cardData.purpose && (
                    <div style={{ fontSize: 11, color: T.muted, fontFamily: font.ui }}>
                      Currently: <strong style={{ color: T.ink }}>{PURPOSE_PRESETS.find(p => p.value === cardData.purpose)?.label || cardData.purpose}</strong>
                    </div>
                  )}
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                  {PURPOSE_PRESETS.map(preset => {
                    const isActive = cardData.purpose === preset.value;
                    return (
                      <button
                        key={preset.value}
                        onClick={() => updateCardPurpose(preset.value)}
                        style={{
                          display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 4,
                          padding: "12px 14px", borderRadius: T.radiusSm, cursor: "pointer",
                          border: `1.5px solid ${isActive ? T.accent : T.border}`,
                          background: isActive ? T.accentSoft : T.surface,
                          transition: "all 0.14s", textAlign: "left", fontFamily: font.ui,
                        }}
                        onMouseEnter={e => { if (!isActive) { e.currentTarget.style.borderColor = T.accent + "88"; e.currentTarget.style.background = T.surfaceAlt; } }}
                      ...
                      >
                        ...
                        <span style={{ fontSize: 12, fontWeight: 700, color: isActive ? T.accent : T.ink, lineHeight: 1.2 }}>{preset.label}</span>
                        <span style={{ fontSize: 10, color: T.muted, lineHeight: 1.3 }}>{preset.desc}</span>
                        {isActive && (
                          <div style={{ marginTop: 2 }}>
                            <div style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "1px 6px", borderRadius: 10, background: T.accent, color: "#fff", fontSize: 9, fontWeight: 700 }}>
                              <Icon name="check" size={9} color="#fff" /> Selected
                            </div>
                          </div>
                        )}
                      </button>
              ...
              <div style={{ background: T.surface, borderRadius: T.radius, border: `1px solid ${T.border}`, padding: 24, marginBottom: 20 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", border: `2px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ width: 10, height: 10, borderRadius: "50%", background: T.muted }} />
                  </div>
                  <h2 style={{ fontSize: 16, fontWeight: 700, color: T.ink, fontFamily: font.display, margin: 0 }}>Card Details</h2>
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
                  <Input label="Card Name *" value={cardData.cardName} onChange={e => updateCardField("cardName", e.target.value)} />
                  <Input label="Email Address" value={cardData.emailAddress} onChange={e => updateCardField("emailAddress", e.target.value)} />
                </div>

                {/* Logo upload */}
                <div style={{ marginBottom: 20 }}>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: T.ink2, marginBottom: 8, fontFamily: font.ui }}>Card Image / Logo</label>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                    <div style={{ width: 80, height: 80, borderRadius: T.radiusSm, overflow: "hidden", border: `1px solid ${T.border}`, flexShrink: 0 }}>
                      {cardData.details.logo ? (
                        <img src={cardData.details.logo} alt="logo" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { e.target.style.display = "none"; }} />
                      ) : (
                        <div style={{ width: "100%", height: "100%", background: T.faint, display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <Icon name="photo" size={24} color={T.muted} />
                        </div>
                      )}
                    </div>
                    <div>
                      <div style={{ fontSize: 11, color: T.muted, marginBottom: 8 }}>Recommended: 600×400px · JPG/PNG</div>
                      <Btn variant="secondary" size="sm" icon={<Icon name="photo" size={13} color={T.sub} />}>Change</Btn>
                    </div>
                  </div>
                </div>

                {/* Contact Button */}
                <div style={{ background: T.surfaceAlt, borderRadius: T.radiusSm, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
                  <Toggle on={cardData.details.enquireButton} onChange={() => updateDetails("enquireButton", !cardData.details.enquireButton)} />
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: T.ink, fontFamily: font.ui }}>Contact Button</span>
                    <span style={{ fontSize: 12, color: T.muted, marginLeft: 8 }}>— Show an enquiry / contact CTA on the card</span>
                  </div>
                  <input
                    value={cardData.details.enquireButtonText}
                    onChange={e => updateDetails("enquireButtonText", e.target.value)}
                    style={{ width: 140, padding: "7px 10px", fontSize: 13, fontFamily: font.ui, border: `1px solid ${T.border}`, borderRadius: T.radiusSm, background: T.surface, color: T.ink, outline: "none" }}
                  />
                </div>
              </div>

              {/* Description & Info block */}
              <div style={{ background: T.surface, borderRadius: T.radius, border: `1px solid ${T.border}`, padding: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 20, height: 20, display: "flex", flexDirection: "column", gap: 3, alignItems: "center", justifyContent: "center" }}>
                      {[0,1,2].map(i => <div key={i} style={{ width: 16, height: 2, background: T.accent, borderRadius: 1 }} />)}
                    </div>
                    <h2 style={{ fontSize: 16, fontWeight: 700, color: T.ink, fontFamily: font.display, margin: 0 }}>Description &amp; Info</h2>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <Toggle on={true} onChange={() => {}} />
                    <span style={{ fontSize: 12, color: T.sub, fontFamily: font.ui }}>Visible on card</span>
                  </div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                  <div>
                    ...
                    <textarea
                      value={cardData.details.description}
                      onChange={e => updateDetails("description", e.target.value)}
                      rows={3}
                      style={{ width: "100%", padding: "9px 12px", fontSize: 13, fontFamily: font.ui, border: `1px solid ${T.border}`, borderRadius: T.radiusSm, background: T.surface, color: T.ink, outline: "none", resize: "vertical", boxSizing: "border-box" }}
                    />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: T.ink2, marginBottom: 4, fontFamily: font.ui }}>Details</label>
                    <textarea
                      value={cardData.details.details}
                ...
                <div>
                  ...
                  <textarea
                    value={cardData.details.information}
                    onChange={e => updateDetails("information", e.target.value)}
                    rows={3}
                    style={{ width: "100%", padding: "9px 12px", fontSize: 13, fontFamily: font.ui, border: `1px solid ${T.border}`, borderRadius: T.radiusSm, background: T.surface, color: T.ink, outline: "none", resize: "vertical", boxSizing: "border-box" }}
                  />
                </div>
          ...
          {leftTab === "contentBlocks" && (
            <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
              ...
              {/* ── TAB LIST (left panel inside content blocks) ── */}
              <div style={{ width: 190, borderRight: `1px solid ${T.border}`, background: T.surfaceAlt, display: "flex", flexDirection: "column", flexShrink: 0 }}>
                <div style={{ padding: "10px 12px", borderBottom: `1px solid ${T.borderLight}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T.muted, fontFamily: font.mono }}>Tabs</span>
                  <button onClick={addTab} style={{ width: 22, height: 22, borderRadius: 5, background: T.accentSoft, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "6px 8px" }}>
                  {tabs.map(tab => (
                    <div
                      ...
                      onClick={() => setSelectedTabId(tab.tabNumber)}
                      style={{
                        display: "flex", alignItems: "center", gap: 6, padding: "8px 10px", borderRadius: T.radiusSm,
                        border: `1px solid ${tab.tabNumber === activeTabId ? T.accent : "transparent"}`,
                        background: tab.tabNumber === activeTabId ? T.surface : "transparent",
                        cursor: "pointer", marginBottom: 2, transition: "all 0.12s",
                      }}
                    >
                      <div style={{ width: 26, height: 26, borderRadius: 6, background: tab.tabNumber === activeTabId ? T.accentSoft : T.faint, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <Icon name="cards" size={12} color={tab.tabNumber === activeTabId ? T.accent : T.muted} />
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        {editingTabId === tab.tabNumber ? (
                          <input
                            autoFocus
                            ...
                            onBlur={() => {
                              if (editingTabName.trim()) {
                                setCardData(prev => ({
                                  ...prev,
                                  cardBlocks: prev.cardBlocks.map(b => b.tabNumber === tab.tabNumber ? { ...b, tabName: editingTabName.trim() } : b),
                                }));
                              }
                            ...
                            style={{ width: "100%", fontSize: 12, fontFamily: font.ui, border: "none", outline: `1px solid ${T.accent}`, borderRadius: 3, padding: "1px 4px", background: T.surface }}
                          />
                        ) : (
                          <div
                            onDoubleClick={e => { e.stopPropagation(); setEditingTabId(tab.tabNumber); setEditingTabName(tab.tabName); }}
                            style={{ fontSize: 12, fontWeight: tab.tabNumber === activeTabId ? 700 : 500, color: tab.tabNumber === activeTabId ? T.ink : T.ink2, fontFamily: font.ui, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}
                          >
                            {tab.tabName}
                          </div>
                        )}
                        <div style={{ fontSize: 10, color: T.muted, fontFamily: font.mono }}>
                          {tabSections.filter(s => s.tabNumber === tab.tabNumber).length} sections
                        </div>
                      </div>
                      <button
                        onClick={e => { e.stopPropagation(); removeTab(tab.tabNumber); }}
                        style={{ background: "none", border: "none", cursor: "pointer", padding: 2, borderRadius: 3, opacity: 0.5, display: "flex" }}
                      >
                ...
                <div style={{ padding: "8px 12px", borderTop: `1px solid ${T.borderLight}`, fontSize: 10, color: T.muted, fontFamily: font.ui, fontStyle: "italic" }}>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: T.ink, fontFamily: font.display }}>{tabs.find(t => t.tabNumber === activeTabId)?.tabName}</div>
                    <div style={{ fontSize: 12, color: T.muted, fontFamily: font.ui }}>{tabSections.length} sections — select a block type per section to control what fields appear</div>
                  </div>
                  <Btn size="sm" icon={<Icon name="plus" size={14} color="#fff" />} onClick={addSection}>Add Section</Btn>
                </div>
                ...
                {tabSections.map(section => {
                  ...
                  return (
                    <div key={section.id} style={{ background: T.surface, borderRadius: T.radius, border: `1px solid ${T.border}`, marginBottom: 12, overflow: "hidden" }}>
                      {/* Section header */}
                      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 16px", background: T.surfaceAlt, borderBottom: isOpen ? `1px solid ${T.borderLight}` : "none" }}>
                        <Icon name="grip" size={14} color={T.muted} />
                        <button onClick={() => toggleSection(section.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex" }}>
                          <Icon name={isOpen ? "chevDown" : "chevRight"} size={14} color={T.sub} />
                        </button>
                        <input
                          value={section.title}
                          onChange={e => updateSectionTitle(section.id, e.target.value)}
                          placeholder="Section heading..."
                          style={{ flex: 1, fontSize: 14, fontWeight: 700, color: T.ink, fontFamily: font.ui, border: "none", background: "transparent", outline: "none", padding: 0 }}
                        />
                        <BlockTypePill
                          blockType={section.blockType || "catalog"}
                          onChange={bt => updateSectionBlockType(section.id, bt)}
                        />
                        <span style={{ fontSize: 11, color: T.muted, fontFamily: font.mono, marginRight: 4 }}>
                          {section.blockType === "layout"
                            ? `${(section.layoutNodes || []).length} blocks`
                            : `${section.propertyRows.length} items`}
                        </span>
                        <button
                          onClick={() => { if (section.blockType !== "layout") addItem(section.id); }}
                          style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 600, color: section.blockType === "layout" ? T.muted : T.accent, background: "none", border: "none", cursor: section.blockType === "layout" ? "default" : "pointer", padding: "4px 8px", borderRadius: 5, fontFamily: font.ui, opacity: section.blockType === "layout" ? 0.4 : 1 }}
                        >
                          <Icon name="plus" size={13} color={section.blockType === "layout" ? T.muted : T.accent} /> Add
                        </button>
                        <button onClick={() => deleteSection(section.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, display: "flex", borderRadius: 5 }}>
                      ...
                      {isOpen && (
                        <div>
                          ...
                          {section.blockType === "layout" ? (
                            <RichLayoutEditor
                              nodes={section.layoutNodes || []}
                              onChange={nodes => updateSectionLayoutNodes(section.id, nodes)}
                            />
                          ) : (
                            <>
                              {section.propertyRows.length === 0 && (
                                <div style={{ padding: "20px", textAlign: "center", color: T.muted, fontSize: 12, fontFamily: font.ui }}>
                                  No items yet. Click <strong>+ Add</strong> to add items.
                                </div>
                              )}
                              {(() => {
                            const bt = BLOCK_TYPES[section.blockType || "catalog"];
                            return section.propertyRows.sort((a, b) => a.sort - b.sort).map((row, rowIdx) => {
                              // Build the main row columns depending on block type
                              const ctrlCols = "24px 24px 24px";
                              const numCol = "20px";
                              let mainCols;
                              if (!bt.hasValue) mainCols = `${numCol} 1fr ${ctrlCols}`;
                              else if (bt.valueFirst) mainCols = `${numCol} 100px 1fr ${ctrlCols}`;
                              else mainCols = `${numCol} 1fr 180px ${ctrlCols}`;

                              const inputBase = { padding: "7px 10px", fontSize: 13, fontFamily: font.ui, border: `1px solid ${T.border}`, borderRadius: T.radiusSm, color: T.ink, outline: "none", background: T.surface, width: "100%", boxSizing: "border-box" };
                              const ctrlBtns = (
                                <>
                                  <button onClick={() => moveItem(section.id, row.id, -1)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, display: "flex", opacity: rowIdx === 0 ? 0.3 : 1 }}>
                                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 15l-6-6-6 6"/></svg>
                                  </button>
                                  <button onClick={() => moveItem(section.id, row.id, 1)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, display: "flex", opacity: rowIdx === section.propertyRows.length - 1 ? 0.3 : 1 }}>
                                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
                                  </button>
                                  <button onClick={() => deleteItem(section.id, row.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, display: "flex" }}>
                              ...
                              return (
                                <div key={row.id} style={{ borderBottom: rowIdx < section.propertyRows.length - 1 ? `1px solid ${T.borderLight}` : "none" }}>
                                  {/* Main row */}
                                  <div style={{ display: "grid", gridTemplateColumns: mainCols, gap: 8, alignItems: "center", padding: "10px 16px" }}>
                                    {/* Row number */}
                                    <div style={{ color: T.muted, fontSize: 11, fontFamily: font.mono, fontWeight: 600, userSelect: "none" }}>{rowIdx + 1}</div>

                                    {/* Value-first block types: show value before heading */}
                                    {bt.hasValue && bt.valueFirst && (
                                      <input
                                        value={row.headingValue || ""}
                                        onChange={e => updateItem(section.id, row.id, "headingValue", e.target.value)}
                                        placeholder={bt.valuePlaceholder}
                                        style={{ ...inputBase, fontFamily: bt.valueMono ? font.mono : font.ui, color: bt.valueMono ? T.accent : T.ink, fontSize: 12 }}
                                      />
                                    )}

                                    {/* Main heading field (always present) */}
                                    <input
                                      value={row.heading || ""}
                                      onChange={e => updateItem(section.id, row.id, "heading", e.target.value)}
                                      placeholder={section.blockType === "schedule" ? "Session / event title" : section.blockType === "property" ? "Feature / attribute" : section.blockType === "checklist" ? "Clause / requirement" : section.blockType === "contact" ? "Name / role" : section.blockType === "info" ? "Heading / title" : "Item name"}
                                      style={inputBase}
                                    />

                                    {/* Value field for non-value-first types */}
                                    {bt.hasValue && !bt.valueFirst && (
                                      <input
                                        value={row.headingValue || ""}
                                        onChange={e => updateItem(section.id, row.id, "headingValue", e.target.value)}
                                        placeholder={bt.valuePlaceholder}
                                        style={{ ...inputBase, fontFamily: bt.valueMono ? font.mono : font.ui, color: bt.valueMono ? T.accent : T.ink }}
                                      />
                                    )}
                                  ...
                                  {/* Description / body row (spans full content width) */}
                                  {bt.hasDesc && (
                                    <div style={{ padding: "0 16px 10px", paddingLeft: `calc(16px + 20px + 8px)` }}>
                                      {(section.blockType === "info") ? (
                                        <textarea
                                          value={row.headingDescription || ""}
                                          onChange={e => updateItem(section.id, row.id, "headingDescription", e.target.value)}
                                          placeholder="Content / body text…"
                                          rows={3}
                                          style={{ width: "100%", padding: "7px 10px", fontSize: 12, fontFamily: font.ui, border: `1px solid ${T.borderLight}`, borderRadius: T.radiusSm, color: T.sub, outline: "none", background: T.surfaceAlt, boxSizing: "border-box", resize: "vertical" }}
                                        />
                                      ) : (
                                        <input
                                          value={row.headingDescription || ""}
                                          onChange={e => updateItem(section.id, row.id, "headingDescription", e.target.value)}
                                          placeholder={section.blockType === "schedule" ? "Venue / speaker (optional)" : section.blockType === "checklist" ? "Details / explanation (optional)" : section.blockType === "contact" ? "Additional info (optional)" : "Description (optional)"}
                                          style={{ width: "100%", padding: "5px 10px", fontSize: 12, fontFamily: font.ui, border: `1px solid ${T.borderLight}`, borderRadius: T.radiusSm, color: T.sub, outline: "none", background: T.surfaceAlt, boxSizing: "border-box" }}
                                        />
                                      )}
                                    </div>
                                  )}
                ...
                {tabSections.length === 0 && (
                  <div style={{ textAlign: "center", padding: "48px 20px", color: T.muted, fontFamily: font.ui }}>
                    <div style={{ fontSize: 32, marginBottom: 12 }}>📋</div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: T.ink2, marginBottom: 6 }}>No sections in this tab</div>
                    <div style={{ fontSize: 13, marginBottom: 20 }}>Add a section to get started</div>
                    <Btn onClick={addSection} icon={<Icon name="plus" size={14} color="#fff" />}>Add Section</Btn>
                  </div>
                )}
              </div>
            </div>
...
export default function PersonalyzAdmin() {
  ...
  const views = {
    ...
    cards: <CardsView onNav={handleNav} />,
    editor: <CardEditor />,
    genericEditor: <GenericCardEditor card={selectedCard} onNav={handleNav} />,
    microsites: <MicrositesView onNav={handleNav} />,
    analytics: <AnalyticsView />,
    users: <UsersView onNav={handleNav} />,
