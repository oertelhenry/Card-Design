...
const StepPersonal = ({ data, onChange }) => {
  const hasCoApplicant = ["Joint","Multiple Applicants"].includes(data.applicantType);
  ...
  return (
    <div style={{animation:"slideUp 0.3s var(--ease-out)"}}>
      ...
      {hasCoApplicant && (
        <ApplicantPersonal title="Co-Applicant / Spouse" role="coapplicant" data={data} onChange={onChange} />
      )}
      {isEntity && (
        <div className="applicant-panel">
          <div className="applicant-head">
            <span style={{fontSize:18}}>🏢</span>
            <span className="h-4">{data.applicantType} Details</span>
          </div>
          <div className="applicant-body">
            <div className="form-row">
            ...
            <SectionDivider label={data.applicantType==="Trust" ? "Trustees / Beneficiaries / Founders" : data.applicantType==="Close Corporation" ? "Members & Beneficial Owners" : "Shareholders & Directors"} />
            <Notif type="info" icon="ℹ️">
              Please list all directors, members, trustees, or shareholders with more than 25% beneficial ownership. At least one entry is required.
            </Notif>
            {[0,1].map(i=>(
              <div key={i} className="form-row3" style={{marginBottom:12}}>
                <Field label={`Full Name ${i+1}`}>
                  <Inp value={data[`entityMember${i}_name`]||""} onChange={e=>onChange(`entityMember${i}_name`,e.target.value)} />
                </Field>
                <Field label="ID Number">
                  <Inp value={data[`entityMember${i}_id`]||""} onChange={e=>onChange(`entityMember${i}_id`,e.target.value)} />
                </Field>
                <Field label="% Holding">
...
const StepEmployment = ({ data, onChange }) => {
  ...
  return (
    <div style={{animation:"slideUp 0.3s var(--ease-out)"}}>
      ...
      {hasCoApplicant && (
        <EmploymentPanel title="Co-Applicant — Employment Details" role="coapplicant" data={data} onChange={onChange} />
      )}
    </div>
  );
};
...
const StepFinancial = ({ data, onChange }) => {
  const hasCoApplicant = ["Joint","Multiple Applicants"].includes(data.applicantType);
  const FinancialPanel = ({ role, title }) => {
    const f = k => `${role}_${k}`;
    return (
      <div className="applicant-panel mb-5">
        <div className="applicant-head">
          <span style={{fontSize:18}}>{role==="applicant"?"👤":"👥"}</span>
          <span className="h-4">{title}</span>
        </div>
  ...
  return (
    <div style={{animation:"slideUp 0.3s var(--ease-out)"}}>
      <FinancialPanel role="applicant" title="Applicant — Financial Details" />
      {hasCoApplicant && <FinancialPanel role="coapplicant" title="Co-Applicant — Financial Details" />}
    </div>
  );
};
...
const StepTax = ({ data, onChange }) => {
  ...
  return (
    <div style={{animation:"slideUp 0.3s var(--ease-out)"}}>
      <Notif type="info" icon="🇿🇦">
        Tax declarations are required under FICA (Financial Intelligence Centre Act) and international FATCA/CRS obligations. All information is transmitted securely and used solely for regulatory compliance.
      </Notif>
      <TaxPanel role="applicant" title="Applicant — Tax Declaration" />
      {hasCoApplicant && <TaxPanel role="coapplicant" title="Co-Applicant — Tax Declaration" />}
    </div>
  );
};
