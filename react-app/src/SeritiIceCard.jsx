...
/* ══════════════════════════════════════════════════════════════
   SERITI SOLUTIONS — "Ice" Digital Business Card
   Recreated from CardDesigns/NewCard3 screen recordings
   (IceMobile.mp4, IceDesktop.mp4)

   Fonts: Playfair Display (serif, italic headings) + system sans
   Palette: smoky dark-green banner, crimson/pink accent + gradient,
            forest-green primary CTA, soft pink decorative blobs
   ══════════════════════════════════════════════════════════════ */

const IMG = {
  banner: "/seriti-ice/Banner.jpg",
  profile: "/seriti-ice/Profile.jpg",
  gallery: [
    "/seriti-ice/Gallery1.jpg",
    "/seriti-ice/Gallery2.jpg",
    "/seriti-ice/Gallery3.jpg",
    "/seriti-ice/Gallery4.jpg",
    "/seriti-ice/Gallery5.jpg",
    "/seriti-ice/Gallery6.jpg",
  ],
};
...
const C = {
  ...
  body: "#4C5878",
  accent: "#C81C46",
  gradient: "linear-gradient(90deg, #EE8798 0%, #C81C46 55%, #E4A177 100%)",
  green: "#356E04",
  greenDeep: "#2A5803",
  iconBg: "#FDE3D9",
  caption: "#B7AD8C",
  blob: "rgba(232,120,140,0.16)",
...
const Icon = ({ name, size = 18, color = "currentColor", sw }) => {
  ...
  switch (name) {
    ...
    case "quote": return <svg width={size} height={size} viewBox="0 0 24 24" fill={color}><path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2-2-2H4c-1.25 0-2 .75-2 2v6c0 1.25.75 2 2 2h1c0 3-1 4-3 4zm11 0c3 0 7-1 7-8V5c0-1.25-.75-2-2-2h-4c-1.25 0-2 .75-2 2v6c0 1.25.75 2 2 2h1c0 3-1 4-3 4z" /></svg>;
    default: return null;
  }
};

/* ── decorative blob ────────────────────────────────────────── */
const Blob = ({ top, bottom, left, right, size = 220 }) => (
  <div style={{
    position: "absolute", top, bottom, left, right, width: size, height: size, borderRadius: "50%",
    background: C.blob, filter: "blur(2px)", pointerEvents: "none",
  }} />
);
...
const Stars = ({ n = 5, size = 12 }) => (
  <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
    ...
    <span style={{ fontSize: 12, fontWeight: 700, color: C.accent, marginLeft: 4 }}>{n.toFixed(1)}</span>
  </div>
);

const Label = ({ children }) => (
  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 8 }}>
    <span style={{ width: 26, height: 1, background: C.accent, opacity: 0.5 }} />
    <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.14em", color: C.accent }}>{children}</span>
    <span style={{ width: 26, height: 1, background: C.accent, opacity: 0.5 }} />
  </div>
);
...
const Heading = ({ children, size = 26 }) => (
  <div style={{ textAlign: "center", marginBottom: 6 }}>
    <h2 style={{ fontFamily: C.serif, fontStyle: "italic", fontWeight: 700, fontSize: size, color: C.navy, margin: 0 }}>{children}</h2>
  </div>
);
...
const PillBtn = ({ icon, label, variant, size }) => {
  ...
  return (
    <button style={{
      ...
      background: solid ? C.accent : "rgba(255,255,255,0.14)",
...
const ServiceCard = ({ item }) => (
  <div className="svc-card" style={{ background: C.cardWhite, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
    <GradientBar />
  ...
  <div style={{ background: C.cardWhite, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
    ...
    <div style={{ position: "relative", aspectRatio: "16/9", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 10, right: 12, opacity: 0.85 }}>
        <Icon name="quote" size={26} color="#fff" />
      </div>
    </div>
    <div style={{ padding: "18px 20px 20px" }}>
      <Stars />
      <p style={{ fontSize: 13.5, lineHeight: 1.65, color: C.body, fontStyle: "italic", margin: "12px 0 16px" }}>&ldquo;{t.quote}&rdquo;</p>
      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 12, display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 30, height: 30, borderRadius: "50%", background: C.iconBg, color: C.accent, fontWeight: 700, fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center" }}>{t.initial}</div>
        <div>
...
const SocialCircle = ({ item, size = 46 }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
    <div style={{ width: size, height: size, borderRadius: 14, background: "#fff", border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
...
/* ══════════════════════════════════════════════════════════════
   HERO — mobile (stacked) & desktop (side-by-side), banner
   covers photo + name + buttons entirely
   ══════════════════════════════════════════════════════════════ */
const Hero = ({ desktop }) => {
  const photo = (
    <div style={{ position: "relative", width: desktop ? 164 : 140, height: desktop ? 198 : 168, flexShrink: 0 }}>
      <img src={IMG.profile} alt={`${PROFILE.first} ${PROFILE.last}`} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 16, display: "block", border: "1px solid rgba(255,255,255,0.35)" }} />
      <div style={{
  ...
  const info = (
    <div style={{ textAlign: desktop ? "left" : "center" }}>
      <h1 style={{ fontFamily: C.serif, fontWeight: 700, fontSize: desktop ? 34 : 25, color: "#fff", margin: "0 0 6px" }}>
        {PROFILE.first} <em style={{ fontStyle: "italic" }}>{PROFILE.last}</em>
      </h1>
      <p style={{ fontSize: desktop ? 15.5 : 14, fontWeight: 700, color: "#fff", margin: "0 0 6px", display: "flex", alignItems: "center", gap: 8, justifyContent: desktop ? "flex-start" : "center" }}>
        <span style={{ width: 14, height: 1.5, background: C.accent, display: "inline-block" }} />
        {PROFILE.title} <span style={{ color: C.accent }}>{PROFILE.region}</span>
      </p>
      <p style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: "0.14em", color: C.caption, margin: "0 0 16px" }}>
        {PROFILE.company}
      </p>
...
const ContentSections = ({ desktop }) => {
  ...
  const wrap = { maxWidth: desktop ? 1200 : 420, margin: "0 auto", padding: desktop ? "0 40px" : "0 20px", position: "relative" };
  ...
  return (
    <>
      {/* ABOUT */}
      <div style={{ background: C.bgLight, padding: sectionPad, position: "relative", overflow: "hidden" }}>
        <Blob top={-60} left={-80} />
        <div style={wrap}>
          <Label>ABOUT</Label>
          <Heading size={desktop ? 32 : 24}>Who We Are</Heading>
          <div style={{
            marginTop: 28, background: C.cardWhite, border: `1px solid ${C.border}`, borderRadius: 18, boxShadow: "0 12px 32px rgba(0,0,0,0.04)",
            padding: desktop ? "36px 44px" : "24px 22px", maxWidth: desktop ? 900 : "none", marginLeft: "auto", marginRight: "auto", position: "relative", overflow: "hidden",
          }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", textAlign: "center", gap: 10 }}>
              {ABOUT.stats.map((s) => (
                <div key={s.l}>
                  ...
                  <div style={{ fontSize: 11.5, color: C.body, marginTop: 2, textTransform: "uppercase", letterSpacing: "0.04em" }}>{s.l}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ background: C.bgLight, padding: sectionPad }}>
        <div style={wrap}>
          ...
          <div style={{ marginTop: 28, maxWidth: 500, margin: "28px auto 0" }}>
            {HOURS.map((row, i) => (
              <div key={row.d} style={{
                display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 4px",
                borderTop: i === 0 ? "none" : `1px solid ${C.border}`,
              }}>
                <span style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13.5, fontWeight: 600, color: row.closed ? "#B0AB96" : C.navy }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: row.closed ? "#C9C2AC" : "#D47F31" }} />
                  {row.d}
                </span>
                {row.closed ? (
                  <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.04em", color: "#9A957F", background: C.bgAlt, borderRadius: 999, padding: "4px 12px" }}>CLOSED</span>
                ) : (
                  <span style={{ fontSize: 13.5, fontWeight: 600, color: C.navy }}>{row.h}</span>
                )}
              </div>
            ))}
          </div>
      ...
      {/* TESTIMONIALS */}
      <div style={{ background: C.bgAlt, padding: sectionPad, position: "relative", overflow: "hidden" }}>
        <Blob top={-40} left={-80} />
        <div style={wrap}>
      ...
      <div style={{ background: C.bgLight, padding: sectionPad, position: "relative", overflow: "hidden" }}>
        ...
        <div style={wrap}>
          ...
          <div style={{
            marginTop: 28, maxWidth: 640, margin: "28px auto 0", background: "#fff", border: `1px solid ${C.border}`, boxShadow: "0 12px 32px rgba(0,0,0,0.04)",
            borderRadius: 18, padding: desktop ? "32px 36px" : "22px 20px", position: "relative",
          }}>
            ...
            <button style={{
              ...
              background: `linear-gradient(180deg, ${C.green}, ${C.greenDeep})`, color: "#fff", fontSize: 14.5, fontWeight: 700, cursor: "pointer",
        ...
        <div style={wrap}>
          ...
          <div style={{ marginTop: 28, display: "grid", gridTemplateColumns: desktop ? "repeat(3,1fr)" : "repeat(2,1fr)", gap: desktop ? 16 : 10 }}>
            {IMG.gallery.map((src, i) => (
              <div key={i} className="gallery-item" style={{ position: "relative", aspectRatio: "1/1", borderRadius: 10, overflow: "hidden" }}>
                <img src={src} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                <div className="gallery-overlay" style={{
                  position: "absolute", inset: 0, background: "rgba(10,8,54,0.45)", opacity: 0,
                  display: "flex", alignItems: "center", justifyContent: "center", color: "#fff",
                  fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", transition: "opacity .15s",
                }}>View</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 28, maxWidth: 900, margin: "28px auto 0", border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden", background: "#fff" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 18px" }}>
              <div style={{ width: 30, height: 30, borderRadius: 9, background: C.iconBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Icon name="mapPin" size={15} color={C.accent} />
              </div>
              <span style={{ fontSize: 13.5, fontWeight: 600, color: C.navy }}>Port Louis, Mauritius</span>
            </div>
            <div style={{ position: "relative", aspectRatio: desktop ? "16/6" : "4/3" }}>
              <iframe
                title="map"
              ...
              <a
                ...
                style={{
                  ...
                  fontSize: 12.5, fontWeight: 700, color: C.navy, textDecoration: "none",
                }}
              >
      ...
      <div style={{ background: C.bgAlt, padding: desktop ? "40px 0" : "32px 0 90px" }}>
        <div style={wrap}>
          <div style={{ display: "flex", flexDirection: desktop ? "row" : "column", justifyContent: "space-between", alignItems: desktop ? "center" : "flex-start", gap: 16 }}>
            <div>
              <div style={{ fontFamily: C.serif, fontWeight: 700, fontSize: 16, color: C.navy }}>SERITI <em style={{ fontStyle: "italic", color: C.accent }}>Solutions</em></div>
              <div style={{ fontSize: 12, color: C.body, marginTop: 2 }}>Innovative Technology Solutions for Financial Services</div>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              {SOCIAL.map((s) => (
                <div key={s.label} style={{ width: 32, height: 32, borderRadius: 10, background: "#fff", border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 20, paddingTop: 16, textAlign: "center" }}>
            <span style={{ fontSize: 12, color: C.body }}>© 2026 SERITI Solutions. All rights reserved.</span>
          </div>
        </div>
      </div>
    </>
...
function MobileCard() {
  return (
    <div style={{ fontFamily: C.sans, position: "relative", background: C.bgLight }}>
      ...
      {/* bottom nav bar with raised WhatsApp FAB */}
      <div style={{
        position: "sticky", bottom: 0, display: "grid", gridTemplateColumns: "repeat(4,1fr)",
        background: "#fff", borderTop: `1px solid ${C.border}`, padding: "10px 0 12px", zIndex: 15, overflow: "visible",
      }}>
        {[["phone", "Call"], ["mail", "Email"], ["whatsapp", "WhatsApp"], ["calendar", "Book"]].map(([ic, label]) => (
          label === "WhatsApp" ? (
            <div key={label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, position: "relative" }}>
              <div style={{
                position: "absolute", top: -34, width: 52, height: 52, borderRadius: "50%", background: C.accent,
                display: "flex", alignItems: "center", justifyContent: "center",
                boxShadow: "0 6px 16px rgba(200,28,70,0.4)", border: "4px solid #fff",
              }}>
                <Icon name="whatsapp" size={18} color="#fff" />
              </div>
              <span style={{ fontSize: 10.5, color: C.accent, fontWeight: 700, marginTop: 22 }}>{label}</span>
            </div>
          ) : (
...
function DesktopCard() {
  return (
    <div style={{ fontFamily: C.sans, position: "relative", background: C.bgLight }}>
      ...
      <button style={{
        position: "fixed", bottom: 28, right: 28, width: 48, height: 48, borderRadius: 16,
        background: `linear-gradient(180deg, ${C.green}, ${C.greenDeep})`, border: "none", display: "flex", alignItems: "center", justifyContent: "center",
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
export default function SeritiIceCard() {
